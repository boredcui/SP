...
Date: 2022-05-13 19:24:10
LastEditors: boredcui 1637188453@qq.com
LastEditTime: 2022-05-13 20:18:00
FilePath: \douban\spider\spidermusic.py
Description: 
...
def main():
    ...
    savepath = "./spider/豆瓣音乐TOP250.xls"
    dbpath = "./spider/musicTop250.db"
    # 3.保存数据
...
findBd = re.compile(r'<p class="pl">(.*?)</p>', re.S)  # 相关内容
...
def saveDate(datalist, savepath):
    ...
    sheet = book.add_sheet('豆瓣音乐TOP250', cell_overwrite_ok=True)  # 创建工作表
    col = ("音乐详情链接", "图片链接", "音乐名", "音乐别名", "评分", "评价数", "相关信息")
    for i in range(0, 7):
...
def saveDateDB(datalist, dbpath):
    ...
    for data in datalist:
        ...
        sql = '''
            insert into music(info_link,pic_link,cname,ename,score,rated,info)
            values(%s)
        ''' % ",".join(data)
        cur.execute(sql)
        conn.commit()
